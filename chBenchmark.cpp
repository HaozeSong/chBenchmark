...
void* analyticalThread(void* args){
	...
	if (Config::getStoreType() == "tikv") {
		SQLHSTMT hStmt = 0;
		SQLAllocHandle(SQL_HANDLE_STMT, hDBC, &hStmt);
		if(!DbcTools::executeServiceStatement(hStmt, "set tidb_isolation_read_engines=\"tikv\"")){
			Log::l1() << Log::tm() << "set tidb_isolation_read_engines = tikv failed";
		}
		SQLFreeHandle(SQL_HANDLE_STMT, hStmt);
	} else if (Config::getStoreType() == "tiflash") {
		SQLHSTMT hStmt = 0;
		SQLAllocHandle(SQL_HANDLE_STMT, hDBC, &hStmt);
		if(!DbcTools::executeServiceStatement(hStmt, "set tidb_isolation_read_engines=\"tiflash\"")){
			Log::l1() << Log::tm() << "set tidb_isolation_read_engines = tiflash failed";
		}
		SQLFreeHandle(SQL_HANDLE_STMT, hStmt);
	}
...
int main(int argc, char* argv[]){
	...
	//2. Run main chBenchmark
		...
		if (Config::getSkipSchemaInit() != 0) {
			Log::l2() << Log::tm() << "Schema init skiped.\n";
		} else {
		...
		if (Config::getSkipSchemaInit() != 0)
			Log::l2() << Log::tm() << "Schema check skiped.\n";
		else {
		...
		resultStream.close();
		Log::l2() << Log::tm() << "-results written to: " << resultFileName << "\n";
